...
/**
 * Store the client instance data in a private WeakMap
 * @type {WeakMap}
 */
...
class DiscogsClient {
    ...
    constructor(userAgent, auth) {
        ...
        clientData.set(this, {
            ...
            auth: {method: 'none', level: 0}
        });
        // Set the custom User Agent when provided
        if (typeof userAgent === 'string') {
            this.config.userAgent = userAgent;
            clientData.get(this).config.userAgent = userAgent;
        }
        ...
        if (auth && (typeof auth === 'object')) {
            ...
            clientData.get(this).auth = Util.merge({}, auth);
        }
    }
    ...
    setConfig(customConfig) {
        let config = clientData.get(this).config;
        Util.merge(this.config, customConfig);
        queue.setConfig({
            maxStack: this.config.requestLimitQueueSize,
    ...
    authenticated(level) {
        ...
        let auth = clientData.get(this).auth;
        return ((auth.level > 0) && (auth.level >= level));
    };
    ...
    about(callback) {
        let clientInfo = {
            ...
            authMethod: clientData.get(this).auth.method,
            authLevel: clientData.get(this).auth.level
        };
     ...
     */
    oauth() {
        return new DiscogsOAuth(clientData.get(this).auth);
    };
...
function rawRequest(options, callback) {
    let data = options.data || null,
        ...
        config = clientData.get(this).config,
        auth = clientData.get(this).auth;
